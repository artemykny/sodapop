...
export function PhaseContent({ state, me, isHost, send, copyInvite, leave }) {
  switch (state.phase) {
    case "answering": return <Answering state={state} send={send} />;
    case "discussion": return <Discussion state={state} isHost={isHost} />;
    case "voting": return <Voting state={state} me={me} send={send} />;
    case "round_result": return <RoundResult state={state} isHost={isHost} />;
